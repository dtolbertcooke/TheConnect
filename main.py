...
def intern_profile(UserID):
    ...
    return render_template('intern_profile.html', UserID=UserID, profile_pic=profile_pic, logo_link=logo_link,
                           edit=edit,
...
def sponsor_profile(UserID):
    ...
    return render_template('sponsor_profile.html', UserID=UserID, profile_pic=profile_pic, company=company,
                           address=address,
                           ...
                           endDate=endDate, fName=fName, lName=lName, degree=degree, gpa=gpa, phone1=phone1,
                           data3=data3)
...
def admin_home():
    ...
    return render_template('admin_home.html', approve_internship_data=approve_internship_data, form_app=form_app,
                           ...
                           contact_data=contact_data, ticket_data=ticket_data,
    ...
    if form.validate_on_submit():
        ...
        c.execute(
            'INSERT INTO Student values("%s","%s","%s","%s","%s","%s","%s",%s,"%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (
                UserID, fname, lname, address, email, phone, major, gpa, state, address2, city, zipcode, interest,
                biography, availability, approved, suggestion))
...
def approve():
    ...
    if request.method == "POST":
        ...
        if 1 <= value_from_AdminHome <= 999:
            db3 = pymysql.connect(host='35.231.51.121', user='root', password='connect1234', db='theConnect')
            cursor68 = db3.cursor()
            sql_approve1 = "UPDATE Student SET approved=%s WHERE UserID=%s"
            cursor68.execute(sql_approve1, (approved, UID))
            db3.commit()
            cursor68.close()
...
def reco():
    db2 = pymysql.connect(host='35.231.51.121', user='root', password='connect1234', db='theConnect')
    cursor67 = db2.cursor()
    clear_value = 1
    print("dndsidjf")
    if request.method == "POST":
        sql_approve1 = "UPDATE Student SET suggestion=0 WHERE suggestion=%s"
        cursor67.execute(sql_approve1, (clear_value))
        db.commit()
    cursor67.close()
    print("we did it")
    return 'hi'
